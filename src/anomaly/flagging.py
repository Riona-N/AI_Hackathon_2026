...
def build_model_event_report(
	...
	"""Build an anomaly report from Member 1's fitted regressor.

	Member 1's pipeline (src/models/train.py) exposes a fitted scikit-learn
	Pipeline with a ``predict`` method. This adapter keeps that handoff
	independent of pandas and preserves optional site/timestamp metadata in
	each report row. In practice, ``build_event_report`` is used directly on
	the standardized predictions table (it already has observed/predicted
	columns) - this adapter is for scoring a new, unlabeled batch straight
